...
# Reconstruction loss
# -----------------------------
def make_reconstruction_loss(weight_scaler, pca, device: str) -> Callable:
    """
    Returns a loss function that maps normalised PCA weights → power spectra,
    then computes MSE in the original (physical) power spectrum space.

    Both pred and target are normalised PCA weights (i.e. model inputs/outputs).
    The inverse transform is:
        weights = pred * scale + mean          (undo StandardScaler)
        spectra = weights @ components + mean  (undo PCA)
    """
    mean_     = torch.tensor(weight_scaler.mean_,   dtype=torch.float32, device=device)
    scale_    = torch.tensor(weight_scaler.scale_,  dtype=torch.float32, device=device)
    components_ = torch.tensor(pca.components_,     dtype=torch.float32, device=device)  # (n_comp, n_k)
    pca_mean_   = torch.tensor(pca.mean_,           dtype=torch.float32, device=device)  # (n_k,)

    def reconstruction_loss(pred_norm: torch.Tensor, target_norm: torch.Tensor) -> torch.Tensor:
        pred_weights   = pred_norm   * scale_ + mean_
        target_weights = target_norm * scale_ + mean_
        pred_spectra   = pred_weights   @ components_ + pca_mean_
        target_spectra = target_weights @ components_ + pca_mean_
        return nn.functional.mse_loss(pred_spectra, target_spectra)
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            valid_loss = float(loss_fn(model(x_val), y_val).item())
        ...
        if trial is not None:
            # report every 10 epochs to reduce SQLite write pressure
            if epoch % 10 == 0:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Optuna tuning for the 21-cm emulator on HPC."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pca_mse",
        choices=["pca_mse", "reconstruction_mse"],
        help=(
            "pca_mse: MSE on normalised PCA weights (default, fast). "
            "reconstruction_mse: MSE on reconstructed power spectra (physical space)."
        ),
    )
    ...
    # Build the loss function once — shared across all trials
    if args.loss == "reconstruction_mse":
        loss_fn = make_reconstruction_loss(
            processed["weight_scaler"], processed["pca"], device
        )
        print("Using reconstruction MSE loss (physical power spectrum space).", flush=True)
    else:
        loss_fn = nn.functional.mse_loss
        print("Using PCA-weight MSE loss (normalised space).", flush=True)
    ...
    storage = args.storage or f"sqlite:///{(args.output_dir / f'{args.study_name}.db').resolve()}?timeout=60"
    ...
    run_tag       = f"run{n_trials_done}_{loss_tag}_{mean_err}"
