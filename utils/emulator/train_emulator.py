...
def train_emulator(
    ...
    processed : dict
        ...
        - "params_train_scaled"       : ndarray of shape (N_train, input_dim)
        - "pca_weights_train_scaled"  : Tensor of shape (N_train, n_comp)
        - "params_val_scaled"         : ndarray of shape (N_val, input_dim)
        - "pca_weights_val_scaled"    : Tensor of shape (N_val, n_comp)
        - "weight_scaler"             : StandardScaler (reconstruction mode only)
        - "pca"                       : PCA object (reconstruction mode only)
    epochs : int, optional
    ...
    x_train = torch.tensor(processed["params_train_scaled"], dtype=torch.float32).to(device)
    y_train = processed["pca_weights_train_scaled"].to(device)
    x_val   = torch.tensor(processed["params_val_scaled"],   dtype=torch.float32).to(device)
    y_val   = processed["pca_weights_val_scaled"].to(device)
